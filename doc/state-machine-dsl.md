...
- 路径生成算法默认 DFS 枚举长度 1 到 `--max-length` 的路径；`--algorithm random` 使用随机游走，`--algorithm tour` 使用转换游。
- `--events e1,e2,...` 按事件序列执行并输出最终状态；若同一状态和事件存在多个转换，当前选择第一条匹配转换。
- `--coverage` 输出状态覆盖和转换覆盖统计；`--cover` 用贪心算法求覆盖全部转换的最小路径集。
- 尚未支持嵌套、复合、历史和并发状态，也未实现 guard 的运行时求值。
