...
- `mutex`：声明两个函数互斥，不能出现在同一个组合调用流程中。
- `constraint`：声明组合约束，目前支持 `count(<函数名>) <比较符> <整数>`，比较符可以是 `<`、`<=`、`>`、`>=`、`==`、`!=`。
...
- `order` 不能形成循环依赖。
- `mutex` 引用的函数必须已声明。
- `constraint` 表达式必须符合 `count(<函数名>) <比较符> <整数>` 格式，且引用的函数已声明。
- 必须存在且仅存在一个初始状态。
