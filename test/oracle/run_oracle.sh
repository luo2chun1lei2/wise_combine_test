...
TOOL="${WISE_ORACLE_TOOL:-$ROOT/src/out/wise_combine_test}"

mkdir -p "$HERE/bin" "$HERE/results"

for mutant in 0 1 2 3 4 5 6; do
    g++ -std=c++17 -O2 -Wall -Wextra -DMUTANT="$mutant" \
        -I"$HERE" "$HERE/adapter.cpp" -o "$HERE/bin/adapter-$mutant"
done
...
for case_id in 1 2 3 4 5 6; do
    for variant in clean mutant; do
        ...
        for repeat in 1 2; do
            ...
            rm -f "$state"
            set +e
            "$TOOL" "$HERE/Q${case_id}.ct" \
                --adapter "$HERE/bin/adapter-$mutant" \
                --adapter-arg "$state" \
                --max-depth 8 \
                --max-flows 200 \
                --no-function-flows \
                --report json >"$output" 2>"$errors"
            rc=$?
            set -e
            ...
            if [ "$expected" = pass ] && [ "$rc" -eq 0 ] && \
                [ "${failed:-x}" = 0 ] && [ "${passed:-0}" -gt 0 ]; then
                verdict=PASS
            fi
            if [ "$expected" = fail ] && [ "$rc" -ne 0 ] && \
                [ "${failed:-0}" -gt 0 ]; then
                verdict=PASS
            fi
            printf '%s\tQ%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
                "$run" "$case_id" "$variant" "$repeat" "$rc" \
                "${passed:-NA}" "${failed:-NA}" "$verdict" \
                >> "$HERE/results/summary.tsv"
        done
    done
done
...
if grep -q 'FAIL$' "$HERE/results/summary.tsv"; then
    exit 1
fi
echo "oracle passed"
