...
StepResult run_step(const generate::Flow& flow, std::size_t index, const model::Transition& transition,
  ...
  if ((result.status == Status::passed || result.status == Status::launch_error) && WIFEXITED(status) && WEXITSTATUS(status) == 0) {
    try {
      const auto response = spec::parse_adapter_response(output);
      result.observed_state = response.observed_state;
      result.returns = response.returns;
      result.stderr_text = response.stderr_output;
      result.status = response.status == "ok" ? Status::passed : (response.status == "mismatch" ? Status::mismatch : Status::adapter_error);
      const auto expected = transition.expect_present ? transition.expect : transition.to;
      if (result.status == Status::passed && !expected.empty() && result.observed_state != expected) {
        result.status = Status::mismatch;
        result.detail = "state mismatch: expected " + expected + ", observed " + result.observed_state;
      } else if (response.status != "ok") result.detail = "adapter reported " + response.status;
      return result;
  ...
  if (result.status == Status::passed || result.status == Status::launch_error) {
    ...
    else { std::string protocol, state, response_status; const auto first = output.find_first_not_of(" \t\r\n"); const auto last = output.find_last_not_of(" \t\r\n"); if (first == std::string::npos || output[first] != '{' || last == std::string::npos || output[last] != '}' || output.find("\"extra\"") != std::string::npos || !field(output, "protocol", protocol) || protocol != "1" || !field(output, "status", response_status) || (response_status != "ok" && response_status != "mismatch" && response_status != "error") || !field(output, "observed_state", state) || output.find("\"stderr\":") == std::string::npos || !parse_returns(output, result.returns)) { result.status = Status::protocol_error; result.detail = "malformed adapter response"; } else { result.observed_state = state == "null" ? "" : state; result.status = response_status == "ok" ? Status::passed : (response_status == "mismatch" ? Status::mismatch : Status::adapter_error); const auto expected = transition.expect_present ? transition.expect : transition.to; if (result.status == Status::passed && !expected.empty() && result.observed_state != expected) { result.status = Status::mismatch; result.detail = "state mismatch: expected " + expected + ", observed " + result.observed_state; } else if (response_status != "ok") result.detail = "adapter reported " + response_status; } }
  }
  return result;
}
}
