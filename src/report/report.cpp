...
std::string json(const runtime::RunResult& r) { std::ostringstream o; o << "{\"flow_id\":\"" << r.flow_id << "\",\"status\":\"" << name(r.status) << "\",\"steps\":["; for (std::size_t i=0;i<r.steps.size();++i) { if(i)o<<','; const auto&s=r.steps[i]; o<<"{\"index\":"<<s.index<<",\"transition\":\""<<s.transition<<"\",\"function\":\""<<s.function<<"\",\"status\":\""<<name(s.status)<<"\",\"observed_state\":\""<<s.observed_state<<"\",\"stderr\":\""<<s.stderr_text<<"\",\"exit_status\":"<<s.exit_status<<",\"detail\":\""<<s.detail<<"\"}"; } return o << "]}", o.str(); }
