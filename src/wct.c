...
static int parse_unsigned_strict(const char *text, unsigned long long *out) {
...
int wct_parse_file(const char *path, wct_state_graph *s, wct_relation_graph *r, char *err, size_t errlen) {
    ...
    FILE *f = fopen(path,"r"); char line[2048]; unsigned schema=0; size_t ln=0, schema_line=0; int state_graph_seen=0, relation_graph_seen=0;
    ...
    while (fgets(line,sizeof line,f)) {
        ...
        else if(!strcmp(tok,"relation_graph")){ if (relation_graph_seen++) { char msg[128]; snprintf(msg,sizeof msg,"line %zu column %zu: duplicate relation_graph",ln,col); seterr(err,errlen,msg); goto fail; } char *id=strtok_r(NULL," \t\r\n",&save); if(!id||strtok_r(NULL," \t\r\n",&save)){char msg[128]; snprintf(msg,sizeof msg,"line %zu column %zu: invalid relation_graph fields",ln, col); seterr(err,errlen,msg);goto fail;} free(r->id); r->id=dupstr(id); if(!r->id)goto oom; }
...
int wct_run_relation(const wct_relation_graph *g, wct_call_fn fn, void *ctx,
    ...
    if (!rr) for (size_t i=0;i<4;i++) { if (lens[i] > WCT_MAX_RESULT) { rr=-1; break; } if (lens[i]) { *dst[i]=malloc((size_t)lens[i]+1); if (!*dst[i] || read_exact_deadline(p[0],*dst[i],(size_t)lens[i],deadline)) { rr=-1; break; } (*dst[i])[lens[i]]='\0'; } }
    close(p[0]); int status=0;
    if (rr == -2) { kill_reap(pid); wct_report_free(r); r->timed_out=1; r->process_signal=SIGKILL; r->error=dupstr("scenario timeout"); return -1; }
    if (rr) { kill_reap(pid); wct_report_free(r); r->error=dupstr("scenario isolation read failed"); return -1; }
