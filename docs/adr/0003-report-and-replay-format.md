...
状态：v2 envelope、完整 payload schema 和 `replay` 已实现并纳入回归。本文与 `wise-combine replay` 的当前行为一致。
...
`verify-report-v2 REPORT` 验证 v2 envelope；v1 只验证结构，v2 先进行严格 envelope 解析并取出
payload 原始字节，计算并比较 SHA-256，然后验证 payload JSON，再验证模型、流程及结果的相互对应关系。成功输出必须明确区分
...
不匹配程序摘要，且不得在验证失败时启动 adapter。启动前必须确认 `ID-0.json`、
`ID-0.txt` 和 `ID-0.v2.json` 都不存在，包括这些路径是符号链接或硬链接的情况。

重放只执行保存的这一条完整流程，不重新运行生成器搜索；仍按原模型进行参数绑定和
状态判定，不能用报告里的 observed 值伪造响应。`NEW_DIR` 下的三条报告输出路径必须全部是
新的不存在路径，禁止覆盖输入报告或其他现有文件。空 result 仅在 `passed` 且流程为空，或
`timeout` 且尚未执行步骤时有效。
