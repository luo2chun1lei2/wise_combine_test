...
- P0 报告路径错误（部分完成）：`0c54f45`、`6107d79`。目录创建失败、流程报告打开/即时写入失败、summary 打开失败返回退出码 `5`；缓冲刷新、关闭及 summary 写入错误仍需补齐检查和故障测试。
...
状态：部分完成。目录创建失败导致异常终止已修复；报告持久化失败处理尚未完成全部验收。原问题是 `--reports` 指向普通文件等不可创建路径时，进程会以 134 终止。

整改：已处理目录创建和文件打开失败；后续必须显式检查写入、刷新和关闭结果，输出路径和原因，返回退出码 `5`，避免写入失败仍输出成功 summary。
